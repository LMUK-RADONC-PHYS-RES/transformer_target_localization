...
import numpy as np
import models.config_TransMorph as config_TransMorph
import time
...
class SpatialTransformer(nn.Module):
    ...
    Jitter from https://github.com/Project-MONAI/MONAI/blob/dev/monai/networks/blocks/warp.py#L105-L110
    """

    def __init__(self, size, mode='bilinear', jitter=False, seed=0):
        super().__init__()
        ...
        if jitter:
            ...
            with torch.random.fork_rng(enabled=seed):
                ...
                grid += torch.rand_like(grid) # add random values from uniform distrubution to grid for regularization
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.spatial_trans = SpatialTransformer(config.img_size, mode='bilinear', jitter=config.jitter)
        self.spatial_trans_seg = SpatialTransformer(config.img_size, mode='nearest', jitter=False)
        self.avg_pool = nn.AvgPool2d(3, stride=2, padding=1)

    def forward(self, moving_img, moving_seg=None, eval_warping=False):
        if self.if_convskip:
            x_s0 = moving_img.clone()
            x_s1 = self.avg_pool(moving_img)
            f4 = self.c1(x_s1)
            f5 = self.c2(x_s0)
        else:
            f4 = None
        ...
        out_feats = self.transformer(moving_img)
        ...
        source = moving_img[:, 0:1, :, :]  # moving image
        out = self.spatial_trans(source, flow) # warped moving image with bilinear interpolation
        
        if moving_seg is None:
            return out, flow
        else:   
            source_seg = moving_seg[:, 0:1, :, :] # moving segmentation
            # t0_forward = time.time()
            if eval_warping:
                # during evaluation warp using nearest neighbour interpolation such that output segmentations are binary
                out_seg = self.spatial_trans_seg(source_seg, flow) 
            else:
                # during training warp moving segmentation with linear interpolation to allow for backprop
                out_seg = self.spatial_trans(source_seg, flow) 
            # t1_forward = time.time()
            # print(f'Time to deform mask: {(t1_forward - t0_forward) * 1000} ms')  # less than 1 ms
            return out, out_seg, flow


dict_model_variants = {
    'TransMorph': config_TransMorph.get_2DTransMorph_config(),
    'TransMorph-With-Jitter': config_TransMorph.get_2DTransMorphWithJitter_config(),
    'TransMorph-No-Conv-Skip': config_TransMorph.get_2DTransMorphNoConvSkip_config(),
    'TransMorph-No-Trans-Skip': config_TransMorph.get_2DTransMorphNoTransSkip_config(),
    'TransMorph-Large': config_TransMorph.get_2DTransMorphLarge_config(),
    'TransMorph-Small': config_TransMorph.get_2DTransMorphSmall_config(),
    'TransMorph-Tiny': config_TransMorph.get_2DTransMorphTiny_config(),
}
